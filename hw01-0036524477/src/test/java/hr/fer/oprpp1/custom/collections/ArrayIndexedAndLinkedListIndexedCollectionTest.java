...
class ArrayIndexedAndLinkedListIndexedCollectionTest {
	...
	/**
	 * A test method that test the forEach method by calling the remove method on
	 * another collection with the same elements
	 */
	@Test
	void testForEachMethod() {
		defaultCollection.add("a");
		defaultCollection.add("b");
		ArrayIndexedCollection helper = new ArrayIndexedCollection(defaultCollection);
		...
		defaultCollection.forEach(new MyProcessor());
		assertArrayEquals(new Object[0], helper.toArray());
	}
